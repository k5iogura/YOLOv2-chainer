...
class YOLOv2(Chain):
    ...
    def __init__(self, n_classes, n_boxes):
        super(YOLOv2, self).__init__(
            ...
            conv1  = L.Convolution2D(3, 32, ksize=3, stride=1, pad=1, nobias=False),
            bn1    = L.BatchNormalization(32, eps=2e-5),
            # bias1  = L.Bias(shape=(32,)),
            conv2  = L.Convolution2D(32, 64, ksize=3, stride=1, pad=1, nobias=False),
            bn2    = L.BatchNormalization(64, eps=2e-5),
            # bias2  = L.Bias(shape=(64,)),
            conv3  = L.Convolution2D(64, 128, ksize=3, stride=1, pad=1, nobias=False),
            bn3    = L.BatchNormalization(128, eps=2e-5),
            # bias3  = L.Bias(shape=(128,)),
            conv4  = L.Convolution2D(128, 64, ksize=1, stride=1, pad=0, nobias=False),
            bn4    = L.BatchNormalization(64, eps=2e-5),
            # bias4  = L.Bias(shape=(64,)),
            conv5  = L.Convolution2D(64, 128, ksize=3, stride=1, pad=1, nobias=False),
            bn5    = L.BatchNormalization(128, eps=2e-5),
            # bias5  = L.Bias(shape=(128,)),
            conv6  = L.Convolution2D(128, 256, ksize=3, stride=1, pad=1, nobias=False),
            bn6    = L.BatchNormalization(256, eps=2e-5),
            # bias6  = L.Bias(shape=(256,)),
            conv7  = L.Convolution2D(256, 128, ksize=1, stride=1, pad=0, nobias=False),
            bn7    = L.BatchNormalization(128, eps=2e-5),
            # bias7  = L.Bias(shape=(128,)),
            conv8  = L.Convolution2D(128, 256, ksize=3, stride=1, pad=1, nobias=False),
            bn8    = L.BatchNormalization(256, eps=2e-5),
            # bias8  = L.Bias(shape=(256,)),
            conv9  = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, nobias=False),
            bn9    = L.BatchNormalization(512, eps=2e-5),
            # bias9  = L.Bias(shape=(512,)),
            conv10 = L.Convolution2D(512, 256, ksize=1, stride=1, pad=0, nobias=False),
            bn10   = L.BatchNormalization(256, eps=2e-5),
            # bias10 = L.Bias(shape=(256,)),
            conv11 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, nobias=False),
            bn11   = L.BatchNormalization(512, eps=2e-5),
            # bias11 = L.Bias(shape=(512,)),
            conv12 = L.Convolution2D(512, 256, ksize=1, stride=1, pad=0, nobias=False),
            bn12   = L.BatchNormalization(256, eps=2e-5),
            # bias12 = L.Bias(shape=(256,)),
            conv13 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, nobias=False),
            bn13   = L.BatchNormalization(512, eps=2e-5),
            # bias13 = L.Bias(shape=(512,)),
            conv14 = L.Convolution2D(512, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn14   = L.BatchNormalization(1024, eps=2e-5),
            # bias14 = L.Bias(shape=(1024,)),
            conv15 = L.Convolution2D(1024, 512, ksize=1, stride=1, pad=0, nobias=False),
            bn15   = L.BatchNormalization(512, eps=2e-5),
            # bias15 = L.Bias(shape=(512,)),
            conv16 = L.Convolution2D(512, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn16   = L.BatchNormalization(1024, eps=2e-5),
            # bias16 = L.Bias(shape=(1024,)),
            conv17 = L.Convolution2D(1024, 512, ksize=1, stride=1, pad=0, nobias=False),
            bn17   = L.BatchNormalization(512, eps=2e-5),
            # bias17 = L.Bias(shape=(512,)),
            conv18 = L.Convolution2D(512, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn18   = L.BatchNormalization(1024, eps=2e-5),
            # bias18 = L.Bias(shape=(1024,)),

            ###### new layer
            conv19 = L.Convolution2D(1024, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn19   = L.BatchNormalization(1024),
            # bias19 = L.Bias(shape=(1024,)),
            conv20 = L.Convolution2D(1024, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn20   = L.BatchNormalization(1024),
            # bias20 = L.Bias(shape=(1024,)),
            conv21 = L.Convolution2D(3072, 1024, ksize=3, stride=1, pad=1, nobias=False),
            bn21   = L.BatchNormalization(1024),
            # bias21 = L.Bias(shape=(1024,)),
            conv22 = L.Convolution2D(1024, n_boxes * (5 + n_classes), ksize=1, stride=1, pad=0, nobias=False),
