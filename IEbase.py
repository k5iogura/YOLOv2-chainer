...
class IE:
    def __init__(self,_xml, _bin, _device, verbose=False):
        ...
        IE.exec_net, IE.plugin, IE.input_blob, IE.out_blobs = IEsetup(_xml, _bin, _device)

    def infer(in_frame):
        result = IEinfer(IE.exec_net, in_frame, IE.input_blob, IE.out_blobs)
        return result
...
def IEfinish(exec_net, plugin, verbose=False):
    if verbose:print("finalizing")
    del exec_net
    del plugin
